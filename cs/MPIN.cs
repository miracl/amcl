...
/* Configure your PIN here */
	...
	public const int HASH_TYPE = SHA256;

	public static sbyte[] mpin_hash(int sha, FP4 c, ECP U)
	{
		sbyte[] w = new sbyte[EFS];
		sbyte[] t = new sbyte[6 * EFS];
		sbyte[] h = null;
		c.geta().A.toBytes(w);
		for (int i = 0;i < EFS;i++)
		{
		...
		c.geta().B.toBytes(w);
		for (int i = EFS;i < 2 * EFS;i++)
		{
			t[i] = w[i - EFS];
		}
		c.getb().A.toBytes(w);
		for (int i = 2 * EFS;i < 3 * EFS;i++)
		{
			t[i] = w[i - 2 * EFS];
		}
		c.getb().B.toBytes(w);
		for (int i = 3 * EFS;i < 4 * EFS;i++)
		{
			t[i] = w[i - 3 * EFS];
		}

		U.X.toBytes(w);
		for (int i = 4 * EFS;i < 5 * EFS;i++)
		{
			t[i] = w[i - 4 * EFS];
		}
		U.Y.toBytes(w);
		for (int i = 5 * EFS;i < 6 * EFS;i++)
		{
			t[i] = w[i - 5 * EFS];
		}
...
/* Hash number (optional) and string to array size of Bigs */
	...
	public static sbyte[] hashit(int sha, int n, sbyte[] B)
	{
		sbyte[] R = null;
		...
		sbyte[] W = new sbyte[ROM.MODBYTES];
		...
		else
		{
			...
			for (int i = sha;i < ROM.MODBYTES;i++)
			{
				W[i] = 0;
			}
		}
...
/* return time in slots since epoch */
	public static int today()
	{
		...
		return (int)(date.Ticks / (1000 * 60 * 1440));
	}
...
/* returns u derived from P. Random value in range 1 to return value should then be added to u */
	...
	public static sbyte[] HASH_ID(int sha, sbyte[] ID)
	{
...
/* Extract PIN from TOKEN for identity CID */
	public static int EXTRACT_PIN(int sha, sbyte[] CID, int pin, sbyte[] TOKEN)
	{
...
/* Implement step 1 on client side of MPin protocol */
	public static int CLIENT_1(int sha, int date, sbyte[] CLIENT_ID, RAND rng, sbyte[] X, int pin, sbyte[] TOKEN, sbyte[] SEC, sbyte[] xID, sbyte[] xCID, sbyte[] PERMIT)
	{
		BIG r = new BIG(ROM.CURVE_Order);
		BIG x;
		if (rng != null)
...
//		byte[] t=new byte[EFS];
		...
		sbyte[] h = hashit(sha,0,CLIENT_ID);
		P = mapit(h);
...
/* Time Permit CTT=S*(date|H(CID)) where S is master secret */
	public static int GET_CLIENT_PERMIT(int sha, int date, sbyte[] S, sbyte[] CID, sbyte[] CTT)
	{
		sbyte[] h = hashit(sha,date,CID);
		ECP P = mapit(h);
...
/* Outputs H(CID) and H(T|H(CID)) for time permits. If no time permits set HID=HTID */
	public static void SERVER_1(int sha, int date, sbyte[] CID, sbyte[] HID, sbyte[] HTID)
	{
		sbyte[] h = hashit(sha,0,CID);
		ECP R , P = mapit(h);
...
/* Pollards kangaroos used to return PIN error */
	public static int KANGAROO(sbyte[] E, sbyte[] F)
	{
		...
		for (j = 0;j < TRAP;j++)
		{
			i = t.geta().geta().A.lastbits(20) % TS;
			t.mul(table[i]);
			dn += distance[i];
		}
		gf.copy(t);
		...
		while (dm - dn < MAXPIN)
		{
			...
			i = ge.geta().geta().A.lastbits(20) % TS;
			ge.mul(table[i]);
			dm += distance[i];
			if (ge.Equals(t))
			{
...
/* Hash the M-Pin transcript - new */
	...
	public static sbyte[] HASH_ALL(int sha, sbyte[] HID, sbyte[] xID, sbyte[] xCID, sbyte[] SEC, sbyte[] Y, sbyte[] R, sbyte[] W)
	{
		int i , tlen = 0;
		sbyte[] T = new sbyte[10 * ROM.MODBYTES + 4];
...
/* wCID = w.(A+AT) */
	public static int CLIENT_KEY(int sha, sbyte[] G1, sbyte[] G2, int pin, sbyte[] R, sbyte[] X, sbyte[] H, sbyte[] wCID, sbyte[] CK)
	{
		sbyte[] t;
...
/* Z=r.A - no time permits involved */
	...
	public static int SERVER_KEY(int sha, sbyte[] Z, sbyte[] SST, sbyte[] W, sbyte[] H, sbyte[] HID, sbyte[] xID, sbyte[] xCID, sbyte[] SK)
	{
		sbyte[] t;
...
/* Generate Y = H(epoch, xCID/xID) */
	public static void GET_Y(int sha, int TimeValue, sbyte[] xCID, sbyte[] Y)
	{
		sbyte[] h = hashit(sha,TimeValue,xCID);
...
/* One pass MPIN Client */
		public static int CLIENT(int sha, int date, sbyte[] CLIENT_ID, RAND RNG, sbyte[] X, int pin, sbyte[] TOKEN, sbyte[] SEC, sbyte[] xID, sbyte[] xCID, sbyte[] PERMIT, int TimeValue, sbyte[] Y)
		{
		  int rtn = 0;
...
/* One pass MPIN Server */
		public static int SERVER(int sha, int date, sbyte[] HID, sbyte[] HTID, sbyte[] Y, sbyte[] SST, sbyte[] xID, sbyte[] xCID, sbyte[] SEC, sbyte[] E, sbyte[] F, sbyte[] CID, int TimeValue)
		{
		  int rtn = 0;
