...
//	j:=0
	...
	var S1 [ED25519.ECDH_EGS]byte
	var W0 [2*ED25519.ECDH_EFS+1]byte
	var W1 [2*ED25519.ECDH_EFS+1]byte
	var Z0 [ED25519.ECDH_EFS]byte
	var Z1 [ED25519.ECDH_EFS]byte
	var SALT [8]byte
	var P1 [3]byte
	var P2 [4]byte
	var V [2*ED25519.ECDH_EFS+1]byte
	var M [17]byte
	var T [12]byte
	var CS [ED25519.ECDH_EGS]byte
	var DS [ED25519.ECDH_EGS]byte
...
/* private key S0 of size EGS bytes derived from Password and Salt */
	...
	S0:=ED25519.PBKDF2(sha,PW,SALT[:],1000,ED25519.ECDH_EGS)
...
/* Calculate common key using DH - IEEE 1363 method */
	...
	ED25519.ECPSVDP_DH(S0,W1[:],Z0[:])
	ED25519.ECPSVDP_DH(S1[:],W0[:],Z1[:])

	same:=true
	for i:=0;i<ED25519.ECDH_EFS;i++ {
	...
	KEY:=ED25519.KDF2(sha,Z0[:],nil,ED25519.ECDH_EAS);
	...
	if ED25519.CURVETYPE!=ED25519.MONTGOMERY {
		...
		C:=ED25519.ECIES_ENCRYPT(sha,P1[:],P2[:],rng,W1[:],M[:],V[:],T[:])
		...
		RM:=ED25519.ECIES_DECRYPT(sha,P1[:],P2[:],V[:],C,T[:],S1[:])
		if RM==nil {
		...
		if ED25519.ECPSP_DSA(sha,rng,S0,M[:],CS[:],DS[:])!=0 {
		...
		if ED25519.ECPVP_DSA(sha,W0[:],M[:],CS[:],DS[:])!=0 {
...
func mpin_BN254(rng *amcl.RAND) {
	...
	const EGS=BN254.MPIN_EGS
	const EFS=BN254.MPIN_EFS
...
func rsa_2048(rng *amcl.RAND) {
	...
	pub:=RSA2048.New_rsa_public_key(RSA2048.FFLEN)
	priv:=RSA2048.New_rsa_private_key(RSA2048.HFLEN)

	var ML [RSA2048.RSA_RFS]byte
	var C [RSA2048.RSA_RFS]byte
	var S [RSA2048.RSA_RFS]byte
