...
int main()
{
	...
	printf("\nInsert this code in FP_mod() in file fp.c between #define UNWOUND and #else \n\n");

	printf("\tt=d[0]; v[0]=((chunk)t*MConst)&BMASK; t+=(dchunk)v[0]*md[0];  s=0; c=(t>>BASEBITS);\n\n");

	for (k=1;k<N;k++)
	{
		printf("\tt=d[%d]+c+s+(dchunk)v[0]*md[%d]",k,k);
		for (i=k-1;i>k/2;i--) printf("+(dchunk)(v[%d]-v[%d])*(md[%d]-md[%d])",k-i,i,i,k-i);
		printf("; v[%d]=((chunk)t*MConst)&BMASK; t+=(dchunk)v[%d]*md[0]; ",k,k);
