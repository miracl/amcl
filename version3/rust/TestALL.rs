...
fn ecdh_ed25519(mut rng: &mut RAND)
{
	...
	ed25519::ecdh::pbkdf2(sha,pp,&salt,1000,EGS,&mut s0);

	print!("Alice's private key= 0x");
	printbinary(&s0);

/* Generate Key pair S/W */
	ed25519::ecdh::key_pair_generate(None,&mut s0,&mut w0);
...
/* Random private key for other party */
	ed25519::ecdh::key_pair_generate(Some(&mut rng),&mut s1,&mut w1);
...
/* Calculate common key using DH - IEEE 1363 method */
	...
	ed25519::ecdh::ecpsvdp_dh(&s0,&w1,&mut z0);
	ed25519::ecdh::ecpsvdp_dh(&s1,&w0,&mut z1);
	...
	ed25519::ecdh::kdf2(sha,&z0,None,EAS,&mut key);
	...
	if ed25519::ecp::CURVETYPE!=ed25519::ecp::MONTGOMERY {
		...
		let cc=ed25519::ecdh::ecies_encrypt(sha,&p1,&p2,&mut rng,&w1,&m[0..17],&mut v,&mut t);
		...
		if let Some(mut c)=cc {
			...
			let mm=ed25519::ecdh::ecies_decrypt(sha,&p1,&p2,&v,&mut c,&t,&s1);
			if let Some(rm)=mm {
		...
		if ed25519::ecdh::ecpsp_dsa(sha,&mut rng,&s0,&m[0..17],&mut cs,&mut ds)!=0 {
		...
		if ed25519::ecdh::ecpvp_dsa(sha,&w0,&m[0..17],&cs,&ds)!=0 {
...
fn mpin_bn254(mut rng: &mut RAND)
{
	...
	const EFS:usize=bn254::mpin::EFS;	
	const EGS:usize=bn254::mpin::EGS;
...
/* Trusted Authority set-up */
	...
	bn254::mpin::random_generate(&mut rng,&mut s);
...
/* Create Client Identity */
	...
	bn254::mpin::hash_id(sha,&client_id,&mut hcid);  /* Either Client or TA calculates Hash(ID) - you decide! */
		
/* Client and Server are issued secrets by DTA */
	bn254::mpin::get_server_secret(&s,&mut sst);
	print!("Server Secret SS: 0x");  printbinary(&sst);	

	bn254::mpin::get_client_secret(&mut s,&hcid,&mut token);
	print!("Client Secret CS: 0x"); printbinary(&token); 

/* Client extracts PIN from secret to create Token */
	let pin:i32=1234;
	println!("Client extracts PIN= {}",pin);
	let mut rtn=bn254::mpin::extract_pin(sha,&client_id,pin,&mut token);
	if rtn != 0 {
	...
	if FULL {
		bn254::mpin::precompute(&token,&hcid,&mut g1,&mut g2);
	}
...
/* Client gets "Time Token" permit from DTA */ 
		bn254::mpin::get_client_permit(sha,date,&s,&hcid,&mut permit);
		print!("Time Permit TP: 0x");  printbinary(&permit);

/* This encoding makes Time permit look random - Elligator squared */
		bn254::mpin::encoding(&mut rng,&mut permit);
		print!("Encoded Time Permit TP: 0x"); printbinary(&permit);
		bn254::mpin::decoding(&mut permit);
...
/* Send U=x.ID to server, and recreate secret from token and pin */
	rtn=bn254::mpin::client_1(sha,date,&client_id,Some(&mut rng),&mut x,pin,&token,&mut sec,Some(&mut xid[..]),Some(&mut xcid[..]),Some(&permit[..]));
	if rtn != 0 {
		println!("FAILURE: CLIENT_1 rtn: {}",rtn);
	}
  
	if FULL {
		bn254::mpin::hash_id(sha,&client_id,&mut hcid);
		bn254::mpin::get_g1_multiple(Some(&mut rng),1,&mut r,&hcid,&mut z);  /* Also Send Z=r.ID to Server, remember random r */
	}
  
/* Server calculates H(ID) and H(T|H(ID)) (if time mpin::PERMITS enabled), and maps them to points on the curve HID and HTID resp. */
		
	bn254::mpin::server_1(sha,date,&client_id,&mut hid,Some(&mut htid[..]));
...
/* Server generates Random number Y and sends it to Client */
	bn254::mpin::random_generate(&mut rng,&mut y);
  
	if FULL {
		bn254::mpin::hash_id(sha,&client_id,&mut hsid);
		bn254::mpin::get_g1_multiple(Some(&mut rng),0,&mut w,&rhid,&mut t);  /* Also send T=w.ID to client, remember random w  */
	}
  
/* Client Second Pass: Inputs Client secret SEC, x and y. Outputs -(x+y)*SEC */
	rtn=bn254::mpin::client_2(&x,&y,&mut sec);
	if rtn != 0 {
...
/* If PIN error not required, set E and F = null */
	...
	if !PINERROR {
		rtn=bn254::mpin::server_2(date,&hid,Some(&htid[..]),&y,&sst,Some(&xid[..]),Some(&xcid[..]),&sec,None,None);
	} else {
		rtn=bn254::mpin::server_2(date,&hid,Some(&htid[..]),&y,&sst,Some(&xid[..]),Some(&xcid[..]),&sec,Some(&mut e),Some(&mut f));
	}

	if rtn == bn254::mpin::BAD_PIN {
		println!("Server says - Bad Pin. I don't know you. Feck off.");
		if PINERROR {
			let err=bn254::mpin::kangaroo(&e,&f);
	...
	if  FULL {
		...
		bn254::mpin::hash_all(sha,&hcid,&xid,pxcid,&sec,&y,&z,&t,&mut h);	
		bn254::mpin::client_key(sha,&g1,&g2,pin,&r,&x,&h,&t,&mut ck);
		print!("Client Key =  0x");  printbinary(&ck);

		bn254::mpin::hash_all(sha,&hsid,&xid,pxcid,&sec,&y,&z,&t,&mut h);			
		bn254::mpin::server_key(sha,&z,&sst,&w,&h,&hid,&xid,pxcid,&mut sk);
...
{
	let sha=rsa2048::rsa::HASH_TYPE;
	let message:&[u8] = b"Hello World\n";
	const RFS:usize=rsa2048::rsa::RFS;

	let mut pbc=rsa2048::rsa::new_public_key(rsa2048::ff::FFLEN);
	let mut prv=rsa2048::rsa::new_private_key(rsa2048::ff::HFLEN);
	...
	rsa2048::rsa::key_pair(&mut rng,65537,&mut prv,&mut pbc);

	println!("Encrypting test string\n");
	rsa2048::rsa::oaep_encode(sha,&message,&mut rng,None,&mut e); /* OAEP encode message M to E  */

	rsa2048::rsa::encrypt(&pbc,&e,&mut c);    /* encrypt encoded message */
	print!("Ciphertext= 0x"); printbinary(&c);

	println!("Decrypting test string");
	rsa2048::rsa::decrypt(&prv,&c,&mut ml);
	let mlen=rsa2048::rsa::oaep_decode(sha,None,&mut ml); /* OAEP decode message  */

	let mess=str::from_utf8(&ml[0..mlen]).unwrap();
	print!("{}",&mess);

	println!("Signing message");
	rsa2048::rsa::pkcs15(sha,message,&mut c); 

	rsa2048::rsa::decrypt(&prv,&c,&mut s);  /* create signature in S */ 

	print!("Signature= 0x"); printbinary(&s);

	rsa2048::rsa::encrypt(&pbc,&s,&mut ms);
	...
	rsa2048::rsa::private_key_kill(&mut prv);
}
