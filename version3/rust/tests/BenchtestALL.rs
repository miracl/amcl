...
fn ed25519(mut rng: &mut RAND)
{
	...
	if ecp::CURVETYPE==amcl::types::CurveType::WEIERSTRASS {
		println!("Weierstrass parameterization");
	}		
	if ecp::CURVETYPE==amcl::types::CurveType::EDWARDS {
		println!("Edwards parameterization");
	}
	if ecp::CURVETYPE==amcl::types::CurveType::MONTGOMERY {
		println!("Montgomery parameterization");
	}

	if fp::MODTYPE==amcl::types::ModType::PSEUDO_MERSENNE {
		println!("Pseudo-Mersenne Modulus");
	}
	if fp::MODTYPE==amcl::types::ModType::MONTGOMERY_FRIENDLY {
		println!("Montgomery friendly Modulus");
	}
	if fp::MODTYPE==amcl::types::ModType::GENERALISED_MERSENNE {
		println!("Generalised-Mersenne Modulus");
	}
	if fp::MODTYPE==amcl::types::ModType::NOT_SPECIAL {
...
fn bn254(mut rng: &mut RAND)
{
	...
	if ecp::CURVE_PAIRING_TYPE==amcl::types::CurvePairingType::BN {
		println!("BN Pairing-Friendly Curve");
	}
	if ecp::CURVE_PAIRING_TYPE==amcl::types::CurvePairingType::BLS {
