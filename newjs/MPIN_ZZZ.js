...
/* Hash number (optional) and string to point on curve */
	...
	mapit: function(h)
	{
		var q=new BIG_XXX(0); q.rcopy(ROM_YYY.Modulus);
		...
		{
			var c=new BIG_XXX(0); c.rcopy(ROM_ZZZ.CURVE_Cof);
			P=P.mul(c);
		}
		return P;
	},
...
/* Fast Hashing to G2 - Fuentes-Castaneda, Knapp and Rodriguez-Henriquez */
		...
		var Fa=new BIG_XXX(0); Fa.rcopy(ROM_ZZZ.CURVE_Fra);
		var Fb=new BIG_XXX(0); Fb.rcopy(ROM_ZZZ.CURVE_Frb);
		X=new FP2_YYY(Fa,Fb); 
		x=new BIG_XXX(0); x.rcopy(ROM_ZZZ.CURVE_Bnx);
...
/* maps a random u to a point on the curve */
	map: function(u,cb)
	{
		var P=new ECP_ZZZ();
		var x=new BIG_XXX(u);
		var p=new BIG_XXX(0); p.rcopy(ROM_YYY.Modulus);
		x.mod(p);
		while (true)
		{
			P.setxi(x,cb);
...
/* create random secret S */
	...
	{
		var r=new BIG_XXX(0); r.rcopy(ROM_ZZZ.CURVE_Order);
...
/* Extract Server Secret SST=S*Q where Q is fixed generator in G2 and S is master secret */
	GET_SERVER_SECRET: function(S,SST)
	{
		...
		var A=new BIG_XXX(0);
		var B=new BIG_XXX(0);
		A.rcopy(ROM_ZZZ.CURVE_Pxa); B.rcopy(ROM_ZZZ.CURVE_Pxb);
		var QX=new FP2_YYY(0); QX.bset(A,B);
		A.rcopy(ROM_ZZZ.CURVE_Pya); B.rcopy(ROM_ZZZ.CURVE_Pyb);
	...
	TEST_PAIR: function(PR)
	{
		...
		var gx=new BIG_XXX(0); gx.rcopy(ROM_ZZZ.CURVE_Gx);	
		var gy=new BIG_XXX(0); gy.rcopy(ROM_ZZZ.CURVE_Gy);
		G.setxy(gx,gy);			
