...
FP_YYY.mod=function(d)
{
	...
	if (FP_YYY.MODTYPE==FP_YYY.PSEUDO_MERSENNE)
	{
		...
		if (ROM_YYY.MConst!=1)
			v=t.pmul(ROM_YYY.MConst);
		else v=0;
		tw=t.w[BIG_XXX.NLEN-1];
		t.w[BIG_XXX.NLEN-1]&=FP_YYY.TMASK;
		t.inc(ROM_YYY.MConst*((tw>>FP_YYY.TBITS)+(v<<(BIG_XXX.BASEBITS-FP_YYY.TBITS))));
		b.add(t);
	}
	...
	if (FP_YYY.MODTYPE==FP_YYY.MONTGOMERY_FRIENDLY)
	{
		for (i=0;i<BIG_XXX.NLEN;i++)
			d.w[BIG_XXX.NLEN+i]+=d.muladd(d.w[i],ROM_YYY.MConst-1,d.w[i],BIG_XXX.NLEN+i-1);
		for (i=0;i<BIG_XXX.NLEN;i++)
	...
	if (FP_YYY.MODTYPE==FP_YYY.NOT_SPECIAL)
	{
		...
		b.copy(BIG_XXX.monty(m,ROM_YYY.MConst,d));
