...
/* Optimal R-ate pairing */
	ate: function(P,Q)
	{
		...
		var x=new BIG_XXX(0); x.rcopy(ROM_ZZZ.CURVE_Bnx);
...
PAIR_ZZZ.glv= function(e)
{
	...
	if (ECP_ZZZ.CURVE_PAIRING_TYPE==ECP_ZZZ.BN)
	{
		var i,j;
		var t=new BIG_XXX(0);
		var q=new BIG_XXX(0); q.rcopy(ROM_ZZZ.CURVE_Order);
		var v=[];

		for (i=0;i<2;i++)
		{
			t.rcopy(ROM_ZZZ.CURVE_W[i]);
...
PAIR_ZZZ.G1mul= function(P,e)
{
	...
	if (ROM_ZZZ.USE_GLV)
	{
		...
		Q.copy(P);
		var q=new BIG_XXX(0); q.rcopy(ROM_ZZZ.CURVE_Order);
		var bcru=new BIG_XXX(0); bcru.rcopy(ROM_ZZZ.CURVE_Cru);
