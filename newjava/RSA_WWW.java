...
public final class RSA_WWW {

	public static final int RFS=BIG_XXX.MODBYTES*FF_WWW.FFLEN;
...
/* generate an RSA key pair */
	...
	public static void KEY_PAIR(RAND rng,int e,rsa_private_key_WWW PRIV,rsa_public_key_WWW PUB)
	{ /* IEEE1363 A16.11/A16.12 more or less */

		int n=PUB.n.getlen()/2;
		FF_WWW t = new FF_WWW(n);
		FF_WWW p1=new FF_WWW(n);
		FF_WWW q1=new FF_WWW(n);
...
/* PKCS 1.5 padding of a message to be signed */
	...
	public static void PRIVATE_KEY_KILL(rsa_private_key_WWW PRIV)
	{
		PRIV.p.zero();
		PRIV.q.zero();
		PRIV.dp.zero();
	...
	public static void ENCRYPT(rsa_public_key_WWW PUB,byte[] F,byte[] G)
	{
		int n=PUB.n.getlen();
		FF_WWW f=new FF_WWW(n);
	...
	public static void DECRYPT(rsa_private_key_WWW PRIV,byte[] G,byte[] F)
	{
		int n=PRIV.p.getlen();
		FF_WWW g=new FF_WWW(2*n);

		FF_WWW.fromBytes(g,G);
		FF_WWW jp=g.dmod(PRIV.p);
		FF_WWW jq=g.dmod(PRIV.q);
