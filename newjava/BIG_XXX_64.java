...
/* Constructors */
	...
	public BIG_XXX(BIG_XXX x)
	{
		for (int i=0;i<NLEN;i++)
			w[i]=x.w[i];
	}

	public BIG_XXX(DBIG_XXX x)
	{
		for (int i=0;i<NLEN;i++)
			w[i]=x.w[i];
	}

	public BIG_XXX(long[] x)
	{
			for (int i=0;i<NLEN;i++)
				w[i]=x[i];
	}
...
/* Conditional swap of two bigs depending on d using XOR - no branches */
	public void cswap(BIG_XXX b,int d)
	{
		int i;
		long t,c=(long)d;
		c=~(c-1);
	...
	public void cmove(BIG_XXX g,int d)
	{
		int i;
		long t,b=-d;
...
/* this*=c and catch overflow in DBIG */
	public DBIG_XXX pxmul(int c)
	{
		DBIG_XXX m=new DBIG_XXX(0);	
		long[] cr=new long[2];
		long carry=0;
...
/* return a*b where result fits in a BIG */
	public static BIG_XXX smul(BIG_XXX a,BIG_XXX b)
	{
		long carry;
		long[] cr=new long[2];
		BIG_XXX c=new BIG_XXX(0);
		for (int i=0;i<NLEN;i++)
		{
			carry=0;
			for (int j=0;j<NLEN;j++)
...
/* return a*b as DBIG */
	public static DBIG_XXX mul(BIG_XXX a,BIG_XXX b)
	{
		DBIG_XXX c=new DBIG_XXX(0);
		long carry;
		long[] cr=new long[2];
//		a.norm();
//		b.norm();
...
/* return a^2 as DBIG */
	public static DBIG_XXX sqr(BIG_XXX a)
	{
		DBIG_XXX c=new DBIG_XXX(0);
		long carry;
		long[] cr=new long[2];
	...
	static BIG_XXX monty(BIG_XXX md,long MC,DBIG_XXX d)
	{
		BIG_XXX b;
		long m,carry;
		long[] cr=new long[2];
...
/* return this+x */
	public BIG_XXX plus(BIG_XXX x) {
		BIG_XXX s=new BIG_XXX(0);
		for (int i=0;i<NLEN;i++)
			s.w[i]=w[i]+x.w[i];	
		return s;
	}
...
/* return this.x */
	public BIG_XXX minus(BIG_XXX x) {
		BIG_XXX d=new BIG_XXX(0);
		for (int i=0;i<NLEN;i++)
			d.w[i]=w[i]-x.w[i];
		return d;
	}
...
/* this-=x */
	public void sub(BIG_XXX x) {
		for (int i=0;i<NLEN;i++)
			w[i]-=x.w[i];
	}

/* reverse subtract this=x-this */
	public void rsub(BIG_XXX x) {
		for (int i=0;i<NLEN;i++)
			w[i]=x.w[i]-w[i];
	}
...
/* convert from byte array to BIG */
	public static BIG_XXX frombytearray(byte[] b,int n)
	{
		BIG_XXX m=new BIG_XXX(0);
	...
	public static BIG_XXX fromBytes(byte[] b)
	{
		return frombytearray(b,0);
	}

/* Compare a and b, return 0 if a==b, -1 if a<b, +1 if a>b. Inputs must be normalised */
	public static int comp(BIG_XXX a,BIG_XXX b)
	{
		for (int i=NLEN-1;i>=0;i--)
		{
...
/* a=1/a mod 2^256. This is very fast! */
	public void invmod2m()
	{
		...
		for (i=8;i<BIGBITS;i<<=1)
		{
			...
			BIG_XXX t1=BIG_XXX.smul(U,b); 
			t1.shr(i);

			c.copy(this); c.shr(i); c.mod2m(i);
			BIG_XXX t2=BIG_XXX.smul(U,c); t2.mod2m(i);
			t1.add(t2);
...
/* get 8*MODBYTES size random number */
	public static BIG_XXX random(RAND rng)
	{
		BIG_XXX m=new BIG_XXX(0);
		int i,b,j=0,r=0;
...
/* Create random BIG in portable way, one bit at a time */
	public static BIG_XXX randomnum(BIG_XXX q,RAND rng) 
	{
		DBIG_XXX d=new DBIG_XXX(0);
		int i,b,j=0,r=0;
...
/* return a*b mod m */
	public static BIG_XXX modmul(BIG_XXX a,BIG_XXX b,BIG_XXX m)
	{
		a.mod(m);
		b.mod(m);
		DBIG_XXX d=mul(a,b);
...
/* return a^2 mod m */
	public static BIG_XXX modsqr(BIG_XXX a,BIG_XXX m)
	{
		a.mod(m);
		DBIG_XXX d=sqr(a);
		return d.mod(m);
...
/* return -a mod m */
	public static BIG_XXX modneg(BIG_XXX a,BIG_XXX m)
	{
		a.mod(m);
		return m.minus(a);
	}
...
/* return this^e mod m */
	public BIG_XXX powmod(BIG_XXX e,BIG_XXX m)
	{
		int bt;
		norm();
		e.norm();
		BIG_XXX a=new BIG_XXX(1);
		BIG_XXX z=new BIG_XXX(e);
		BIG_XXX s=new BIG_XXX(this);
		while (true)
		{
			bt=z.parity();
			z.fshr(1);
...
/* Jacobi Symbol (this/p). Returns 0, 1 or -1 */
	public int jacobi(BIG_XXX p)
	{
		int n8,k,m=0;
		BIG_XXX t=new BIG_XXX(0);
		BIG_XXX x=new BIG_XXX(0);
		BIG_XXX n=new BIG_XXX(0);
		BIG_XXX zilch=new BIG_XXX(0);
...
/* this=1/this mod p. Binary method */
	public void invmodp(BIG_XXX p)
	{
		mod(p);
		BIG_XXX u=new BIG_XXX(this);
		BIG_XXX v=new BIG_XXX(p);
		BIG_XXX x1=new BIG_XXX(1);
		BIG_XXX x2=new BIG_XXX(0);
		BIG_XXX t=new BIG_XXX(0);
		BIG_XXX one=new BIG_XXX(1);
